...
export function ImportProspectsModal({ isOpen, onClose, onSuccess }: ImportProspectsModalProps) {
  const { t } = useApp();
      ...
      complete: async (results) => {
        try {
          ...
          const payload = rows.map((row) => {
            // Find name, email, phone flexibly
          ...
          setError(err.message || 'Erro ao importar prospectos. Verifique o formato do arquivo.');
        } finally {
          setLoading(false);
        }
      },
      ...
      <div className="bg-white rounded-3xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <div className="px-6 py-5 border-b border-[#EDF0F4] flex items-center justify-between bg-[#F8FAFB]">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-[#2F4251]">Importar Prospectos</h2>
              <p className="text-xs text-[#8A9BB0]">Faça upload de um CSV com seus contatos frios.</p>
            </div>
          </div>
        ...
        <div className="px-6 py-4 border-t border-[#EDF0F4] flex items-center justify-end gap-3 bg-white">
          ...
          >
            {loading ? (
              ...
              <><CheckCircle2 size={16} /> Importar Base</>
            )}
          </button>
        </div>
      </div>

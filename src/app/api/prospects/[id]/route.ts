...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (body.action === 'convert_to_lead') {
    ...
    if (fetchErr) return NextResponse.json({ error: 'Prospect not found' }, { status: 404 });
    if (prospect.status === 'converted') return NextResponse.json({ error: 'Already converted' }, { status: 400 });
    ...
    await supabaseAdmin.rpc('log_activity', {
      ...
      p_actor_id: userId,
      p_type: 'import',
      p_description: 'Prospect convertido em Lead.',
      p_metadata: { from_prospect_id: prospect.id }
    });

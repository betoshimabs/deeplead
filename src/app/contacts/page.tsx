...
import { fetchContacts, createContact, updateContact, deleteContact, convertContactToLead } from '@/lib/services/contacts';
import type { ContactFilters } from '@/lib/services/contacts';
...
export default function ContactsPage() {
  ...
  const loadContacts = useCallback((f: ContactFilters, p: number) => {
    setLoading(true);
    fetchContacts({ ...f, limit: PAGE_SIZE, offset: p * PAGE_SIZE })
      .then((res: any) => {
        setContacts(res.data ?? []);
        setTotal(res.total ?? 0);
      })
      .catch(() => alert('Erro ao carregar contatos.'))
  ...
  useEffect(() => {
    const to = setTimeout(() => {
      startTransition(() => {
        setPage(0);
        loadContacts({ ...filters, q: searchInput }, 0);
      });
    }, 400);
    return () => clearTimeout(to);
  }, [searchInput, filters, loadContacts]);

  useEffect(() => {
    loadContacts({ ...filters, q: searchInput }, page);
  }, [page, loadContacts]); // Removed filters and searchInput to avoid double fetch

  const handleConvert = async (id: string) => {
    if (!confirm('Deseja converter este Contato em Lead qualificado?')) return;
    setConvertingId(id);
    try {
  ...
  const handleExport = () => {
    if (contacts.length === 0) return alert('Não há contatos para exportar.');
    
    const exportData = contacts.map(p => ({
      Nome: p.name || '',
      Email: p.email || '',
      Telefone: p.phone || '',
      Origem: p.source || '',
    ...
    link.href = url;
    link.setAttribute('download', `contatos_export_${formatDistanceToNow(new Date())}.csv`);
  ...
  const stats = {
    total: total,
    new: contacts.filter(p => p.status === 'new').length,
    converted: contacts.filter(p => p.status === 'converted').length,
  };
    ...
    <AppLayout title="Contatos">
      ...
      <div className="flex items-center gap-3 mb-5 flex-wrap">
        ...
        <div className="flex items-center gap-2">
          <button 
            onClick={() => alert('Em breve: Automação + IA para enriquecimento de contatos')}
          ...
          >
            <Wand2 size={14} /> Enriquecer contatos
          </button>
          ...
          >
            <Upload size={14} /> Importar contatos
          </button>
          ...
          <button 
            ...
            className="px-3.5 py-2 bg-white text-[#555D6F] rounded-xl text-sm font-semibold hover:bg-[#F4F7FA] hover:text-[#2F4251] transition-all flex items-center gap-1.5 border border-[#DAE1EA]"
          >
            <Download size={14} /> Exportar contatos
          </button>

          <button 
            onClick={() => setCreateOpen(true)}
            className="px-3.5 py-2 bg-[#127284] text-white rounded-xl text-sm font-semibold hover:bg-[#0E5B6A] transition-all flex items-center gap-1.5"
          >
      ...
      <div className="bg-white border border-[#DAE1EA] rounded-2xl shadow-sm flex flex-col flex-1 overflow-hidden min-h-[400px]">
        <div className="flex items-center px-4 py-3 bg-[#F4F7FA] border-b border-[#EDF0F4] text-[11px] font-bold text-[#8A9BB0] uppercase tracking-wider shrink-0">
          <div className="w-8" />
          <div className="flex-1 min-w-[200px]">Contato</div>
        ...
        <div className="flex-1 overflow-y-auto min-h-0 bg-white">
          {loading && contacts.length === 0 ? <Skeleton /> : null}
          {!loading && contacts.length === 0 && (
            <div className="flex flex-col items-center justify-center py-20 text-center px-4">
              ...
              <h3 className="text-[#2F4251] font-semibold mb-1">Nenhum contato encontrado</h3>
              <p className="text-[#8A9BB0] text-sm max-w-sm">A base de contatos está vazia ou não há resultados para a busca.</p>
            </div>
          )}
        ...
        <div className="px-4 py-3 bg-white border-t border-[#EDF0F4] flex items-center justify-between shrink-0">
          <div className="text-xs text-[#8A9BB0]">
            Mostrando <span className="font-semibold text-[#2F4251]">{Math.min(total, contacts.length)}</span> de <span className="font-semibold text-[#2F4251]">{total}</span>
          </div>
          <div className="flex items-center gap-2">
      ...
      {createOpen && (
        <div className="fixed inset-0 bg-[#2F4251]/40 z-50 flex items-center justify-center p-4 backdrop-blur-[2px]">
          <div className="bg-white rounded-2xl w-full max-w-md p-6 shadow-2xl">
            <h2 className="font-semibold text-lg text-[#2F4251] mb-4">Adicionar Contato</h2>
